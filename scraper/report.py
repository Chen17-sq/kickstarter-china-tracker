...
def days_since(epoch) -> int | None:
    if not epoch:
        return None
    try:
        return max(0, int((dt.datetime.now(dt.UTC).timestamp() - float(epoch)) / 86400))
...
def days_until(epoch) -> int | None:
    if not epoch:
        return None
    try:
        return max(0, int((float(epoch) - dt.datetime.now(dt.UTC).timestamp()) / 86400))
    except (TypeError, ValueError):
        return None
...
def fmt_epoch_date(epoch) -> str:
    if not epoch:
        return "—"
    try:
        return dt.datetime.fromtimestamp(float(epoch), tz=dt.UTC).strftime("%Y-%m-%d")
    except (TypeError, ValueError):
        return "—"
...
def make_report(curr: dict, prev: dict | None) -> str:
    ...
    out.append("```")
    out.append("")
    out.append("# Kickstarter China Tracker")
    out.append("")
    ...
    if live:
        ...
        out.append("✦ &nbsp; ✦ &nbsp; ✦")
        out.append("")
        out.append("## Section D · 🔴 Live · Top 10")
        out.append("")
        # Top 3 detail blocks
    ...
    if successful:
        ...
        out.append("✦ &nbsp; ✦ &nbsp; ✦")
        out.append("")
        out.append("## Section E · ✅ 最近已结束 · Top 10")
        out.append("")
    ...
    out.append("---")
    out.append("")
    out.append("*All the news that's fit to print, every morning at 08:00 Beijing.*")
    out.append("")
